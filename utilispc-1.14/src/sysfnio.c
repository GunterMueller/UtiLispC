...
WORD intern_soft_f(int na, WORD *fp)
{
  ...
  pushb(value(obvector));
  value(obvector)=obv;
  rval=intern_soft((char *)stringcodes(str),fixtoi(osize(str)),fp);
  val=popb();
  value(popb())=val;
  return rval;
}
...
WORD inopen_f(int na, WORD *fp)
{
  ...
  if(mode(a)&STRINGSTREAM){
    ...
    if((fd=open((char *)stringcodes(b),O_RDONLY))<0){
      openerr(a,fp);
    }
    fildes(a)=fd;
  }
...
WORD appendopen_f(int na, WORD *fp)
{
  ...
  if(mode(a)&STRINGSTREAM)
    ...
    if((fd=open((char *)stringcodes(b),O_WRONLY))<0)
      openerr(a,fp);
    lseek(fd,0,2);
    fildes(a)=fd;
  }
...
WORD outopen_f(int na, WORD *fp)
{
  ...
  if(mode(a)&STRINGSTREAM){
    ...
    if((fd=open((char *)stringcodes(b),O_TRUNC|O_WRONLY|O_CREAT,0666))<0){
      openerr(a,fp);
    }
    fildes(a)=fd;
  }
